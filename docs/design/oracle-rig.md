...
4. **Trace plumbing** (session): `gbx-oracle` (format types, canonical
   ...
   synthetic goldens in CI. **LANDED 2026-07-15** (`crates/gbx-oracle`:
   `format.rs`/`compare.rs`/`sink.rs`, `frontends/cli/src/trace_compare.rs`,
   `crates/gbx-oracle/tests/golden.rs`): the `.gbxtrace` types + a
   byte-deterministic canonical JSONL writer (serde/serde_json, integers only,
   fixed field order) + a liberal parser (ignores the hook's extra diagnostic
   fields); the projection comparator (validity gate over
   `gbxtrace`/`profile`/`game`/`seed`/`encounter`; equality over
   `(before, after)` extended to `(n, result)` when both sides carry them;
   `caller` and all diagnostics excluded) with located divergence diagnostics;
   the chain-continuity check (`after_i == Prng::next(before_i)`,
   `before_{i+1} == after_i`, `randomize` = loud finding) with per-break draw
   index; the engine sink (a `RngSink` trait on the *engine* side —
   `gbx-engine` never depends on `gbx-oracle` — hooked at `EngineRng::random`,
   zero-cost/inert when unattached, never serialized so the `.rsav` golden is
   unchanged) and its `gbx-oracle` `TraceCollector` implementation; the thin
   `restrike trace-compare` CLI; and a synthetic restrike-vs-restrike SHA-256
   golden (D10-clean, generated by running, independently re-derived). The
   `action` profile's *mechanism* (profile tag, versioning, same-tick emission
   order) exists; its event *vocabulary* is deliberately left for the combat
   sessions (step 5). Only step 3 (the staging hook, Bryan + Fable) and step 5
   (combat) remain before H4's rng-stream half can close.
