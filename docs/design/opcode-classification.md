...
| 0x25 | ON GOTO | `CMD_OnGotoGoSub` ovr003.cs:1032-1064 (branch `gbl.command==0x25`) | `vm_LoadCmdSets(2)` (1034) → `ecl_offset--` (1037) → `vm_LoadCmdSets(var_2)` (1038), `var_2` = decoded byte, **data-dependent 0-255** | 0 | **YES** | MACHINE | none | `ecl_offset` (jump target) | none | Declared size 0 but runtime consumes `2+var_2` operand slots — an IF-false landing here desyncs the pc by the whole real operand span. |
| 0x26 | ON GOSUB | `CMD_OnGotoGoSub` ovr003.cs:1032-1064 (branch `gbl.command==0x26`) | same as 0x25 | 0 | **YES** | MACHINE | none | `vmCallStack.Push(ecl_offset)` (1055) in addition to the jump | none | Same mismatch shape as 0x25, plus the unbounded call-stack push. |
...
| 0x2D | CALL | `CMD_Call` ovr003.cs:1832-1910 | 1×1 (fixed) | 1 | NO | MACHINE (dispatch), SVC, EFF | Dispatches on `var_4 = cmd_opps[1].Word - 0x7fff` (raw Word) — a **hidden second syscall table**, at least 6 cases: map-geometry query (`get_wall_x2`/`getMap_wall_type`); `RedrawView`+`display_map_position_time`; `SetupDuel(bool)` (ovr008.cs:1305, clones `SelectedPlayer` into a hostile NPC); `PlaySound` (referenced, not read); `MovePositionForward` (ovr008.cs:1256, writes `mapPosX/Y`/`positionChanged` directly, bypassing ScriptMemory addressing); `DrawMaybeOverlayed`+frame-advance+`GameDelay` | `mapWallRoof/mapWallType`, `combat_type`, `TeamList`, `mapPosX/Y`, `positionChanged` | none found (`GameDelay` appears to be a timed sleep, not input-wait — UNSURE, not fully verified) | **Docket candidate**: CALL is effectively its own opcode table keyed on `operand-0x7FFF`; the EngineServices classification should enumerate each case as its own micro-service rather than one opaque "Call". |
...
### CALL (0x2D) — flagged as needing its own enumerated case list
CALL dispatches on `operand.Word - 0x7FFF` as a hidden second table (at least
6 cases found: map query, redraw, duel setup, sound, move-forward,
overlay-draw-with-delay). Recommend the M1 EngineServices declaration treat
each case as its own named method (`setup_duel`, `move_position_forward`,
etc., several already listed above) rather than one opaque `call(code, args)`
passthrough — otherwise the D-VM4 channel seam is understated for this
opcode. Full case enumeration is a docket item (not all 6 cases were traced
to completion within the timebox).
...
8. **SURPRISE (0x23) writes a hard-coded literal address `0x2CB`**, not an
   ...
   wall info).
9. **CALL (0x2D) is a hidden second dispatch table** keyed on
   `operand.Word-0x7FFF`; not all cases were traced to completion within the
   timebox (map-geometry query naming in particular is UNSURE). Needs a
   follow-up pass before the EngineServices trait is finalized for this
   opcode.
