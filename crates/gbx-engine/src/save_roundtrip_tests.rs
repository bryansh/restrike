...
fn golden_hash_of_a_synthetic_rsav_is_stable() {
    ...
    // Recomputed for SAVE_FORMAT_VERSION 7 (roll-credits slice 6, death
    // recovery — the slice's ONE break). `EngineState` gained the two `Area2`
    // shop flags `CMD_Combat`'s non-monster branch dispatches on
    // (`enter_temple` @0x5C4, `enter_shop` @0x6D8). They are persisted rather
    // than `#[serde(skip)]`ped because the original's own `SaveGame` writes
    // `area2_ptr` whole (`ovr017.cs:1109-1156`), flags included. Both the
    // version bytes and the payload moved. Regenerated by running this test
    // and reading the actual digest — not hand-edited.
    //
    // Recomputed for SAVE_FORMAT_VERSION 8 (roll-credits slice 7,
    // wilderness/overworld — the slice's ONE break). `PictureLayer` gained
    // `last_dax_block` (`gbl.lastDaxBlockId`, the byte the city-scene guard's
    // four sites read), and `EngineState` lost `area_view_allowed` — FD-41's
    // `block_area_view` is now read from its live ScriptMemory cell. Both the
    // version bytes and the payload moved. Regenerated by running this test
    // and reading the actual digest — not hand-edited.
    // The same v8 break also turned `GameClock` from a scalar `total_units`
    // into `step_game_time`'s own seven-word record — same slice, same bump,
    // one recompute.
    //
    // Recomputed for SAVE_FORMAT_VERSION 9 (roll-credits slice 9a): the
    // version bytes moved, and `Screen::Shop` now carries the real
    // `shop_screen::ShopHost` instead of the M3 placeholder. Regenerated by
    // running this test and reading the actual digest — not hand-edited.
    const GOLDEN: &str = "1324f80a9f10df2c467a6cf647bc9b5bae03482a2b9247b4121492e90e3354db";
